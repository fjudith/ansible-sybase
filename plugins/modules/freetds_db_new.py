...
FREETDS_IMP_ERR = None
try:
    import pyodbc
except ImportError:
    FREETDS_IMP_ERR = traceback.format_exc()
    freetds_found = False
else:
    freetds_found = True
...
def main():
    ...
    if not freetds_found:
        module.fail_json(msg=missing_required_lib('pymssql'), exception=freetds_IMP_ERR)
    ...
    try:
        conn = pyodbc.connect(user=login_user, password=login_password, host=login_querystring, database='master')
        cursor = conn.cursor()
